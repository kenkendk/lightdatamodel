...
namespace System.Data.LightDatamodel
{
	...
	public abstract class DataClassExtended : DataClassBase, IDataClass
	{
		...
		protected SortedList<string, Guid> m_referenceObjects = new SortedList<string, Guid>();
		...
		protected object GetReferenceObject(string propertyname, string idfieldname)
		{
		    ...
		    if (!found)
		    {
				...
				DataClassExtended c = (DataClassExtended)m_dataparent.GetObjectById(pi.PropertyType, fi.GetValue(this));
		        if (c == null)
		            m_referenceObjects[propertyname] = Guid.Empty;
		        else
		            m_referenceObjects[propertyname] = c.Guid;
		    }

			return (m_dataparent as DataFetcherCached).GetObjectByGuid(m_referenceObjects[propertyname]);
		}
		...
		protected void SetReferenceObject(string propertyname, string idfieldname, string reversePropertyname, object value)
		{
			if (value == null)
			{
				if (reversePropertyname != null)
				{
					object revObj = GetReferenceObject(propertyname, idfieldname);
					if (revObj != null)
					{
			...
			if ((value as DataClassExtended).m_dataparent != this.m_dataparent)	throw new Exception("Cannot mix objects from differenct data contexts");

			m_referenceObjects[propertyname] = (value as DataClassExtended).m_guid;
			...
			if (fi == null)
				throw new Exception("Bad config for reference object, field " + idfieldname + " does not exist in class " + GetType().FullName);
			m_isdirty = true;
			fi.SetValue(this, (value as DataClassExtended).UniqueValue);
