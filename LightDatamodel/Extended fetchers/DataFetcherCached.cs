...
namespace System.Data.LightDatamodel
{
	...
	public class DataFetcherCached : DataFetcher, IDataFetcherCached
	{
		...
		public override DATACLASS[] GetObjects<DATACLASS>(QueryModel.Operation operation)
		{
            ...
            QueryModel.Operation filter = Query.Equal(Query.SubProperty(Query.FunctionCall("GetType"), "FullName"), Query.Value(typeof(DATACLASS).FullName));
		...
		public override object[] GetObjects(Type type, QueryModel.Operation operation)
		{
            ...
            QueryModel.Operation filter = Query.Equal(Query.SubProperty(Query.FunctionCall("GetType"), "FullName"), Query.Value(type.FullName));
		...
		{
			return GetObjectsFromCache <DATACLASS>(Query.Parse(filter, parameters));
		}
		...
		{
			return GetObjectsFromCache(type, Query.Parse(filter, parameters));
		}
        ...
        protected object[] InsertObjectsInCache(object[] data)
        {
            ...
            foreach (object item in data)
            {
                ...
                if (item as DataClassBase != null)
                {
                    ...
                    {
                        //Query.Parse("GetType().FullName = ? AND UniqueValue = ?", item.GetType().FullName, dbitem.UniqueValue);
                        QueryModel.Operation opdeleted =
                            Query.And(
                                Query.Equal(Query.SubProperty(Query.FunctionCall("GetType"), "FullName"), Query.Value(item.GetType().FullName)),
		...
		public override object GetObjectById(Type type, object id)
		{
			...
			{
                QueryModel.Operation op = Query.Equal(Query.Property(m_transformer.TypeConfiguration.GetTypeInfo(type).PrimaryKey.ColumnName), Query.Value(id));
			...
			else
			{
				...
				filter = Query.Equal(Query.Property(m_transformer.TypeConfiguration.GetTypeInfo(type).PrimaryKey.ColumnName), Query.Value(id));
				lst = filter.EvaluateList(lst);
				if (lst.Count == 1)
					return lst[0];
				else
					return null;
