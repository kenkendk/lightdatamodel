...
namespace System.Data.LightDatamodel
{
	...
	public class DataFetcherCached : DataFetcher, IDataFetcherCached
	{
		...
		*/
		...
		/// <returns></returns>
		public virtual DATACLASS[] GetReferenceObjects<DATACLASS>(string properyName, string idProperty, DataClassExtended refObj) where DATACLASS : IDataClass
		{
			Type type = typeof(DATACLASS);
			string tablename = type.Name;
			if (!m_knownTypes.ContainsKey(tablename))
			...
			return GetObjectsFromCache < DATACLASS>(properyName + ".Guid=?" + refObj.Guid);
		}
		...
		public virtual DATACLASS GetObjectByGuid<DATACLASS>(object key) where DATACLASS : IDataClass
		{
			if (key == null || key.GetType() != typeof(Guid))
				return default(DATACLASS);

			if (m_guidCache.ContainsKey(key))
				return (DataClassExtended)m_guidCache[key];
		...
		public override DATACLASS GetObjectById<DATACLASS>(object id)
		{
			...
			return (DATACLASS)((Hashtable)m_cache[tablename])[id];
		}
