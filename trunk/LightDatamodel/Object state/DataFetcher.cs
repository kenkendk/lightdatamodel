...
namespace System.Data.LightDatamodel
{
	...
	public class DataFetcher : IDataFetcher
	{
        ...
        {
            m_provider.DeleteRow(obj.GetType().Name, ((IDataClass)obj).UniqueColumn, ((IDataClass)obj).UniqueValue);
        }
        ...
        {
            m_provider.InsertRow(obj.GetType().Name, ObjectTransformer.GetDataFromObject(obj));
            FieldInfo fi = obj.GetType().GetField("m_" + ((IDataClass)obj).UniqueColumn, BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.DeclaredOnly);
            if (fi != null && (MemberModifier.CalculateModifier(fi) & MemberModifierEnum.AutoIncrement) != 0)
                fi.SetValue(obj, m_provider.GetLastAutogeneratedValue(obj.GetType().Name));
        }
        ...
        {
            m_provider.UpdateRow(obj.GetType().Name, ((IDataClass)obj).UniqueColumn, ((IDataClass)obj).UniqueValue, ObjectTransformer.GetDataFromObject(obj));
        }
        ...
        {
            Data[][] values = m_provider.SelectRows(type.Name, op);
            object[] res = new object[values.Length];
            for (int i = 0; i < res.Length; i++)
                res[i] = ObjectTransformer.PopulateDataClass(Activator.CreateInstance(type), values[i], m_provider);
            return res;
        }
